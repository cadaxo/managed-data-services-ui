sap.ui.define([
	...
	Dialog, DialogType, HorizontalLayout, VerticalLayout, MessageToast, ActionButton) {
	"use strict";
	...
	return Controller.extend("com.cadaxo.cmds.controller.App", {
	  ...
	  onInit: function() {
		...
		oController._displayFieldActive = false;
		...
		var oMenu = new sap.m.Menu({
			items: [
				new sap.m.MenuItem({
					key: '1',
					text: 'Display Field Source'
				}),
				new sap.m.MenuItem({
					key: '2',
					text: 'Context Menu Item2'
				}),
			]
		})
		var oContextMenuModel = new JSONModel({menu: oMenu});
		this.getView().setModel(oContextMenuModel, "contextMenu");
		...
		var oMessageTemplate = new sap.m.MessageItem({
			type: '{messagePopoverModel>type}',
			title: '{messagePopoverModel>title}',
			activeTitle: "{messagePopoverModel>active}",
			description: '{messagePopoverModel>description}',
			subtitle: '{messagePopoverModel>subtitle}',
			counter: '{messagePopoverModel>counter}',
			link: oLink
		});
		...
		var aMockMessages = [{
			type: 'Success',
			title: 'ZCDX_PARTNER_FIELDS',
			description: 'First Success message description',
			subtitle: 'Display Field Source - First Name + expand',
			counter: 1
		}, {
			type: 'Error',
			title: 'ZCDX_PARTNER_FIELDS',
			description: 'Second Error message description',
			subtitle: 'Display Field Source - First Name',
			counter: 2
		}, {
			type: 'Information',
			title: 'ZCDX_PARTNER_FIELDS',
			description: 'First Information message description',
			subtitle: 'Mode: Show Database Tables',
			counter: 1
		}];
	  ...
	  menuItemPressed: function(oEvent) {
		MessageBox.information(oEvent.getSource().getCustomData()[0].getValue());
	  ...
	  openSidebar: function(oNode) {
		...
		oController.getView().byId("tree-fields").setContextMenu(new sap.m.Menu({
			items: [
				new sap.m.MenuItem({text: "Display Field Source", icon: "sap-icon://arrow-right", press: oController.menuItemPressed}),
				new sap.m.MenuItem({text: 'Item with Custom Data', customData: new sap.ui.core.CustomData({key:'cd1', value:"{nodeDetail>text}"}), press: oController.menuItemPressed}),
				new sap.m.MenuItem({text: "Item Loooooooooooooooong Teeeeeeeeeeeeest"})
			]
		}));
	...
	getFieldSourcePressed: function(oEvent) {
		...
		const fnSuccessGraphData = function(oData, oResponse) {
			...
			var allNodes = this.getModel('step2Model').getData();
			var allLinks = this.getModel('step2ModelLinks').getData();
			const oJson = new JSONModel({new: 'test'});
			oJson.setSizeLimit(Number.MAX_SAFE_INTEGER);
			oJson.setData({
				nodes: allNodes,
			...
			var oActionButton = new ActionButton({
				...
				press: oController.get3MoreFields
			});


			var oNewNode = new sap.suite.ui.commons.networkgraph.Node({
				key: '123456789',
				icon: 'sap-icon://overflow',
				title: '3 more objects',
			...
			oController._graph.addNode(oNewNode);

			oController._graph.addLine(new sap.suite.ui.commons.networkgraph.Line({
				from: '202D23CF499BC4567191AE30B0721FB69F4CCE09',
				to: '123456789',
			...
			oController._graph.addLine(new sap.suite.ui.commons.networkgraph.Line({
				from: '123456789',
				to: '122C19583ECEFA0E7363D37D5314FDB855CC2F42',
				arrowOrientation: 'None'
			}));


			oController.getView().byId("myText").setText("Mode: Display Field Source - FirstName");
		}
		...
		get3MoreFields: function (oEvent) {

			const fnSuccessGraphData = function(oData, oResponse) {	

			var allNodes = this.getModel('step3Model').getData();
			var allLinks = this.getModel('step3ModelLinks').getData();
			const oJson = new JSONModel({new: 'test'});
			oJson.setSizeLimit(Number.MAX_SAFE_INTEGER);
			oJson.setData({
				nodes: allNodes,
		...
		handlePopoverPress: function (oEvent) {
			...
			var oPopover = new sap.m.Popover({
				...
				placement: "Bottom",
				content: [
					new sap.m.Text({text: 'Display Field Source - Desctiption \n Switch to Free Mode - Desctiption'})
				]
